...
#include <stdbool.h>
#include "x86-64-ufispace-s9600-72xc-cpld.h"
...
enum s9600_cpld_sysfs_attributes {
    CPLD_ACCESS_REG,
    CPLD_REGISTER_VAL,
    CPLD_SKU_ID,
    CPLD_HW_REV,
...
static const struct i2c_device_id s9600_cpld_id[] = {
    { "s9600_72xc_cpld1",  cpld1 },
    { "s9600_72xc_cpld2",  cpld2 },
    { "s9600_72xc_cpld3",  cpld3 },
    { "s9600_72xc_cpld4",  cpld4 },    
    {}
};

/* Addresses scanned for s9600_cpld */
...
/* cpld 1 */
static struct attribute *s9600_cpld1_attributes[] = {
...
/* cpld 2 */
static struct attribute *s9600_cpld2_attributes[] = {
    &sensor_dev_attr_cpld_access_register.dev_attr.attr,
    &sensor_dev_attr_cpld_register_value.dev_attr.attr,
    &sensor_dev_attr_cpld_major_ver.dev_attr.attr,
    &sensor_dev_attr_cpld_minor_ver.dev_attr.attr,
...
/* cpld 3 */
static struct attribute *s9600_cpld3_attributes[] = {
    &sensor_dev_attr_cpld_access_register.dev_attr.attr,
    &sensor_dev_attr_cpld_register_value.dev_attr.attr,
    &sensor_dev_attr_cpld_major_ver.dev_attr.attr,
    &sensor_dev_attr_cpld_minor_ver.dev_attr.attr,
...
/* cpld 4 */
static struct attribute *s9600_cpld4_attributes[] = {
    &sensor_dev_attr_cpld_access_register.dev_attr.attr,
    &sensor_dev_attr_cpld_register_value.dev_attr.attr,
    &sensor_dev_attr_cpld_major_ver.dev_attr.attr,
    &sensor_dev_attr_cpld_minor_ver.dev_attr.attr,
...
static const struct attribute_group s9600_cpld1_group = {
    .attrs = s9600_cpld1_attributes,
};
/* cpld 2 attributes group */
static const struct attribute_group s9600_cpld2_group = {
    .attrs = s9600_cpld2_attributes,
};
/* cpld 3 attributes group */
static const struct attribute_group s9600_cpld3_group = {
    .attrs = s9600_cpld3_attributes,
};
/* cpld 4 attributes group */
static const struct attribute_group s9600_cpld4_group = {
    .attrs = s9600_cpld4_attributes,
};
...
static void s9600_cpld_add_client(struct i2c_client *client)
{
...
static void s9600_cpld_remove_client(struct i2c_client *client)
{
...
/* cpld drvier probe */
static int s9600_cpld_probe(struct i2c_client *client,
                    const struct i2c_device_id *dev_id)
{
    ...
    s9600_cpld_add_client(client);

    return 0;
exit:
    switch (data->index) {
    case cpld1:
        sysfs_remove_group(&client->dev.kobj, &s9600_cpld1_group);
        break;
    case cpld2:    	  
    	  sysfs_remove_group(&client->dev.kobj, &s9600_cpld2_group);
        break;
    case cpld3:
    	  sysfs_remove_group(&client->dev.kobj, &s9600_cpld3_group);
        break;
    case cpld4:
    	  sysfs_remove_group(&client->dev.kobj, &s9600_cpld4_group);
        break;
...
/* cpld drvier remove */
static int s9600_cpld_remove(struct i2c_client *client)
{
    struct cpld_data *data = i2c_get_clientdata(client);
    ...
    s9600_cpld_remove_client(client);
    return 0;
}

MODULE_DEVICE_TABLE(i2c, s9600_cpld_id);

static struct i2c_driver s9600_cpld_driver = {
    .class      = I2C_CLASS_HWMON,
    .driver = {
        .name = "x86_64_ufispace_s9600_72xc_cpld",
    },
    .probe = s9600_cpld_probe,
    .remove = s9600_cpld_remove,
    .id_table = s9600_cpld_id,
    .address_list = cpld_i2c_addr,
};

/* provide cpld register read */
/* cpld_idx indicate the index of cpld device */
int s9600_cpld_read(u8 cpld_idx,
                u8 reg)
{
    struct list_head *list_node = NULL;
    struct cpld_client_node *cpld_node = NULL;
...
EXPORT_SYMBOL(s9600_cpld_read);

/* provide cpld register write */
/* cpld_idx indicate the index of cpld device */
int s9600_cpld_write(u8 cpld_idx,
                u8 reg,
                u8 value)
{
    struct list_head *list_node = NULL;
...
EXPORT_SYMBOL(s9600_cpld_write);

static int __init s9600_cpld_init(void)
{
    mutex_init(&list_lock);
    return i2c_add_driver(&s9600_cpld_driver);
}

static void __exit s9600_cpld_exit(void)
{
    i2c_del_driver(&s9600_cpld_driver);
}

MODULE_AUTHOR("Leo Lin <leo.yt.lin@ufispace.com>");
MODULE_DESCRIPTION("s9600_cpld driver");
MODULE_LICENSE("GPL");

module_init(s9600_cpld_init);
module_exit(s9600_cpld_exit);
